/**
 * Solana wallet integration — extension popup.
...
export async function sendSol(input: {
  ...
  try {
    ...
    if (/insufficient funds|insufficient lamports/i.test(msg)) {
      throw new SolanaSendError('insufficient', 'Insufficient SOL balance');
    }
    if (/blockhash|node is behind/i.test(msg)) {
      throw new SolanaSendError('rpc_error', 'Solana RPC issue — try again');
    }
