/**
 * LAX card client (extension) — talks to the Thanos backend LAX proxy
 * (services/api/src/routes/lax.ts), never to LAX/FCFpay directly. The
 * partner API key is a server-side secret; the extension only ever calls
 * its own backend, and every /lax/* route is behind requireAuth, so a
 * Thanos account (apiClient session) is required — the "Create" screen of
 * the card flow registers one.
 *
 ...
 *
 * Ported 1:1 from apps/mobile/lib/lax.ts — same function names/signatures,
 * swapping mobile's apiClient (./auth-client) for the extension's own
 * (also ./auth-client, same @thanos/api-client under the hood).
 */
