...
import { Check, ChevronLeft, Plus, CreditCard, Shield, MoreHorizontal, AlertTriangle } from 'lucide-react';
import {
  laxStatus, laxCards, laxCardTransactions, laxCurrencies, laxTopUp,
  laxRegisterThanosAccount, hasThanosAccount, cardNumberOf,
  type LaxStatus, type LaxCard as LaxCardT, type LaxTxn,
} from '../lib/lax';
...
function LaxCreate({ status, onDone }: { status: LaxStatus | null; onDone: () => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <React.Fragment key={s}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{
                ...
                <span style={{ color: i === 0 ? '#fff' : 'var(--text-muted)', fontSize: 12, fontWeight: 800 }}>{i + 1}</span>
              </div>
              <span style={{ color: i === 0 ? 'var(--text-primary)' : 'var(--text-muted)', fontSize: 11 }}>{s}</span>
            </div>
      ...
      <button className="btn-primary" onClick={submit} disabled={!agreed || busy} style={{ width: '100%' }}>
        {busy ? 'Creating…' : 'Next'}
      </button>
      <p style={{ color: 'var(--text-muted)', fontSize: 11, textAlign: 'center', lineHeight: 1.45, margin: 0 }}>
...
function LaxDashboard({
  card, last4, txns, notLive, onTopUp, onSoon,
}: {
  card: LaxCardT | null; last4?: string; txns: LaxTxn[]; notLive: boolean;
  onTopUp: () => void; onSoon: () => void;
}) {
  const balance = card?.balance != null ? Number(card.balance) : null;
  const actions = [
    { icon: Plus,               label: 'Top Up',       onClick: notLive ? onSoon : onTopUp },
    { icon: CreditCard,         label: 'Card Details', onClick: onSoon },
    { icon: Shield,             label: 'Freeze',       onClick: onSoon },
    { icon: MoreHorizontal,     label: 'More',         onClick: onSoon },
  ];
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
