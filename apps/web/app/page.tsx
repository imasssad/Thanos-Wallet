...
/* Thanos product video (apps/web/public/video.mp4) — plays inline in a modal
   lightbox; the user never leaves the page. Swap PRODUCT_VIDEO to change it. */
const PRODUCT_VIDEO = '/video.mp4';
...
function WatchVideo() {
  ...
  if (!PRODUCT_VIDEO) return null;
  return (
    <>
      <button type="button" className="lp-btn-ghost lp-btn-video" onClick={() => setOpen(true)}>
        <PlayIcon size={15} /> Watch video
      </button>
      {open && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Thanos product video"
        >
          <div className="lp-video-box" onClick={(e) => e.stopPropagation()}>
            ...
            <video className="lp-video-frame" src={PRODUCT_VIDEO} controls autoPlay playsInline preload="metadata" />
          </div>
        </div>
      )}
    </>
...
const ANDROID_APK_VERSION = '1.1.2';
