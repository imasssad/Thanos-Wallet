/**
 * Hide/show networks + assets (web) — a pure DISPLAY preference. It never
 * restricts Send/Receive/dApp use, so a hidden asset stays fully reachable
 * once un-hidden; only the portfolio list respects it. Mirrors the mobile /
 ...
 *
 * localStorage is synchronous, so the cache just hydrates at import time —
 * SSR-safe via a typeof-window guard (this module is only ever used from
 * 'use client' components, but Next can still evaluate it during a server
 * render pass).
 */
...
export function toggleNetworkVisibility(key: string): void {
  ...
  try { window.localStorage.setItem(HIDDEN_NETWORKS_KEY, JSON.stringify([...hiddenNetworks])); } catch { /* ignore */ }
}
