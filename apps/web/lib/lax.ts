/**
 * LAX card client (web) — talks to the Thanos backend LAX proxy
 ...
 * own (./auth-client, localStorage) which wraps the same @thanos/api-client.
 */
  ...
  try {
    ...
    try {
      if (typeof window !== 'undefined') {
        window.localStorage.setItem(CURRENCIES_CACHE_KEY, JSON.stringify({ data, at: Date.now() }));
      }
