/**
 * MultX cross-chain bridge glue (desktop renderer).
 *
 * Same shape as web/extension: `@litho/multx-sdk` is ethers **v5**, the app is
 * ethers **v6**. The vendored tarball nests ethers@5 under the SDK; we build the
 * v5 `Signer` from the `ethers5` alias. The renderer already derives the EVM key
 * from the unlocked seed for sends, so the bridge does the same and binds a v5
