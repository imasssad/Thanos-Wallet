/**
 * MultX bridge client — desktop renderer.
...
export async function getStatus(idOrHash: string): Promise<Status> {
  return json<Status>('GET', `/bridge/status/${encodeURIComponent(idOrHash)}`);
}
